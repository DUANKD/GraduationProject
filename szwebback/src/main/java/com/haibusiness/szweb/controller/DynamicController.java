...
public class DynamicController {
    ...
    @GetMapping(value = "/public/dynamic/{id}")
    public ResponseEntity getDynamic(@PathVariable Long id) {
        Dynamic dynamic = null;
        try {
            dynamic = dynamicService.getDynamicById(id);
        } catch (Exception e) {
            return ResponseEntity.ok().body(new Response(false, "获取数据失败"));
        }
        return ResponseEntity.ok().body(dynamic);
    }
