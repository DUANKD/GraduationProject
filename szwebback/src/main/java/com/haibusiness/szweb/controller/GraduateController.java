...
public class GraduateController {
    ...
    @GetMapping(value = "/public/graduate/{id}")
    public ResponseEntity getGraduate(@PathVariable Long id) {
        Graduate graduate = null;
        try {
            graduate = graduateService.getGraduateById(id);
        } catch (Exception e) {
            return ResponseEntity.ok().body(new Response(false, "获取数据失败"));
        }
        return ResponseEntity.ok().body(graduate);
    }
