...
public class CollaborationController {
    ...
    public ResponseEntity list(
        ...
        model.put("page", page);
        model.put("title", menuService.getMenus().get(type));
        model.put("type", type);
        model.put("name", menuService.getMenus().get("collaboration"));
        return ResponseEntity.ok().body(model);
    }


    @GetMapping(value = "/public/collaboration/{id}")
    public ResponseEntity getCollaboration(@PathVariable Long id) {
        Collaboration collaboration = null;
        try {
            collaboration = collaborationService.getCollaborationById(id);
        } catch (Exception e) {
