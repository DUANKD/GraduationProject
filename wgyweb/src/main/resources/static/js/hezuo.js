...
// 获取编辑合作的界面
$("#rightContainer").on("click", "#editHezuo", function () {

    var url = $(this).attr("url");

    $.ajax({
        url: url + $(this).attr("hezuoId"),
        success: function (data) {
...
$("#submitEdit").click(function () {
    ...
    $.ajax({
        url: url + type,
        type: 'POST',
        data: $('#hezuoForm').serialize(),
        success: function (data) {
            $('#hezuoForm')[0].reset();

            if (data.success) {
                // 从新刷新主界面
                getArticleByTitle(0, _pageSize, '/hezuo/list/' + type);
            } else {
...
// 删除用户
$("#rightContainer").on("click", "#deleteHezuo", function () {
    //获取 CSRF Token
